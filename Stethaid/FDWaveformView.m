    ...
    [self.asset loadValuesAsynchronouslyForKeys:@[@"duration"] completionHandler:^() {
        ...
        switch (durationStatus) {
            ...
            case AVKeyValueStatusUnknown:
                NSLog(@"AV Key Value Status Unknown");
            case AVKeyValueStatusLoading:
                NSLog(@"AV Key Value Status Loading");
                ...
                done:(void(^)(UIImage *image, UIImage *selectedImage))done
{
    ...
    if (normalizeMax == noiseFloor) { //TRIAL CODE
        sampleAdjustmentFactor = imageHeight*2; //TRIAL CODE
        NSLog(@"normalizeMax was equal to noiseFloor");
    }
    else {sampleAdjustmentFactor = imageHeight / (normalizeMax - noiseFloor) / 2;}
    NSLog(@"sampleAdjustmentFactor: %f", sampleAdjustmentFactor);
    
    sampleAdjustmentFactor = 10.546179; //TRIAL CODE
    NSLog(@"sampleAdjustmentFactor: %f", sampleAdjustmentFactor);
